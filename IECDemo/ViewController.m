...
- (void)viewDidLoad {
    ...
    [IceClientUtil serviceAsync:^{
        systemManageServicePrx = (systemmanageSystemManageServicePrx*)[IceClientUtil getServicePrx:@"SystemManageService" class:[systemmanageSystemManageServicePrx class]];
        if ([systemManageServicePrx isKindOfClass:[systemmanageSystemManageServicePrx class]]) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^ {
                [self exception:[NSString stringWithFormat:@"%ld,%@",result.status,result.message]];
            });
